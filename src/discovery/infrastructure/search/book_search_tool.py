...
class BookSearchTool:
    ...
    def as_tool(self) -> Any:
        ...
        async def search_books_tool(query: str) -> list[dict[str, Any]]:
            ...
            국내(한국)에 정식 출간(정발)되어 국내 서점(알라딘 등)에서 검색 가능한
            도서, 그중에서도 2000년 이후 출간작을 우선적으로 찾도록 쿼리를
            구성하세요. 해외 미출간 원서나 2000년 이전 고전문학은 검색 결과에
            나오더라도 추천에서 제외해야 합니다.

            Args:
                query: 도서 검색을 위한 질의. '국내 출간'/'한국 정발' 같은 표현을
                    포함해 국내 출간작 위주로 검색하세요
                    (예: "비 오는 날 읽기 좋은 소설 국내 출간", "불편한 편의점 저자 쪽수 페이지수").
            """
