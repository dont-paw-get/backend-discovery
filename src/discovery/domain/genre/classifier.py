...
반드시 아래 정의된 16개 표준 장르 중 가장 적합한 1개를 선택하세요.


[16개 표준 장르 목록]
1. SF: SF, 공상과학, 사이버펑크, 스페이스 오페라 등
2. 판타지: 판타지, 무협, 다크판타지, 어반판타지, 로맨스판타지 등
3. 로맨스: 로맨스, 순정, 연애소설 등
4. 미스터리/스릴러: 추리, 미스터리, 스릴러, 서스펜스, 공포, 호러, 범죄소설 등
5. 순수소설/일반소설: 한국소설, 영미소설, 고전문학, 일반문학, 장편/단편소설, 테마소설 등
6. 에세이: 수필, 산문집, 그림에세이, 일상에세이, 여행에세이 등
7. 시/희곡: 시집, 희곡, 대본집, 시론 등
8. 인문학: 철학, 심리학, 윤리학, 언어학, 신화/종교학(학술), 고전인문 등
9. 역사: 한국사, 세계사, 동양사, 서양사, 고고학, 역사인물/사건 등
10. 경제/경영: 경영학, 경제학, 마케팅/세일즈, 재테크, 주식/투자, 비즈니스 등
11. 자기계발: 성공/처세, 인간관계, 화술/협상, 시간관리, 습관, 리더십, 동기부여 등
12. 과학: 자연과학, 수학, 물리학, 화학, 생명과학, 지구과학, 천문학, 교양과학 등
13. 예술: 미술, 음악, 디자인, 사진, 건축, 영화/드라마/공연, 만화(기법), 공예 등
14. 종교: 기독교, 불교, 가톨릭, 이슬람교, 종교일반, 신앙/명상 등
15. 컴퓨터/IT: 프로그래밍, 개발, IT교양, 컴퓨터공학, 인공지능/데이터, 네트워크/보안 등
16. 기타: 위 장르에 해당하지 않는 잡지, 수험서, 여행가이드, 요리, 취미/실용, 유아, 만화 등
...
3. 소설 중에서 세부 장르(SF, 판타지, 로맨스, 미스터리/스릴러) 구분이 뚜렷하지 않은
   일반 문학이나 고전문학 등은 '순수소설/일반소설'로 분류하세요.
4. 어떤 범주에도 명확히 부합하지 않거나 도서 정보가 너무 부족한 경우 '기타'로 분류하세요.

[출력 형식]
반드시 다른 설명이나 마크다운 코드블록 없이, 오직 아래의 유효한 JSON 형식 단 1개만 출력하세요:
{"genre": "<16개 표준 장르 중 정확한 명칭>", "confidence": <0.0부터 1.0 사이의 분류 신뢰도 실수>}
"""
...
def build_classification_prompt(title: str, author: str = "", raw_category: str = "") -> str:
    ...
    return (
        ...
        f"위 도서를 분석하여 16개 표준 장르 중 가장 적합한 1개와 신뢰도를 JSON 형식으로 반환하세요."
    )
...
def match_standard_genre(genre_str: str) -> StandardGenre | None:
    ...
    for g in StandardGenre:
        if g.value == genre_str.strip().strip("\"'"):
            return g

    # 2. Enum name 매칭 (예: SF, FANTASY, IT_COMPUTER 등)
    try:
    ...
    exact_alias_map: dict[str, StandardGenre] = {
        "SF": StandardGenre.SF,
        "공상과학": StandardGenre.SF,
        "SCIENCE FICTION": StandardGenre.SF,
        ...
        "순수소설": StandardGenre.GENERAL_FICTION,
        "일반소설": StandardGenre.GENERAL_FICTION,
        "한국소설": StandardGenre.GENERAL_FICTION,
        "영미소설": StandardGenre.GENERAL_FICTION,
        "세계소설": StandardGenre.GENERAL_FICTION,
        "고전문학": StandardGenre.GENERAL_FICTION,
        "소설": StandardGenre.GENERAL_FICTION,
        "에세이": StandardGenre.ESSAY,
        "수필": StandardGenre.ESSAY,
        "산문집": StandardGenre.ESSAY,
        "시/희곡": StandardGenre.POETRY_PLAY,
        "시": StandardGenre.POETRY_PLAY,
        "희곡": StandardGenre.POETRY_PLAY,
        ...
        "경제": StandardGenre.BUSINESS_ECONOMY,
        "경영": StandardGenre.BUSINESS_ECONOMY,
        "경영학": StandardGenre.BUSINESS_ECONOMY,
        "재테크": StandardGenre.BUSINESS_ECONOMY,
        "투자": StandardGenre.BUSINESS_ECONOMY,
        ...
        "미술": StandardGenre.ART,
        "음악": StandardGenre.ART,
        "디자인": StandardGenre.ART,
        ...
        "컴퓨터": StandardGenre.IT_COMPUTER,
        "IT": StandardGenre.IT_COMPUTER,
        "프로그래밍": StandardGenre.IT_COMPUTER,
        "소프트웨어": StandardGenre.IT_COMPUTER,
        "개발": StandardGenre.IT_COMPUTER,
        "기타": StandardGenre.ETC,
        "ETC": StandardGenre.ETC,
    }
...
def parse_classification_response(raw_text: str) -> BookClassificationResponse:
    ...
    if not raw_text or not raw_text.strip():
        logger.warning("장르 분류 LLM 응답이 비어 있어 '기타'로 대체합니다.")
    ...
    if json_match:
        try:
            ...
            if mapped_genre is not None:
                ...
                logger.warning(
                    "알 수 없는 장르 문자열('%s')로 인해 '기타'로 대체합니다. 원본: %s",
                    raw_genre,
                    raw_text,
                )
                return BookClassificationResponse(genre=StandardGenre.ETC, confidence=0.0)
    ...
    logger.warning("장르 분류 파싱 실패. '기타'로 fallback합니다: %s", raw_text)
    return BookClassificationResponse(genre=StandardGenre.ETC, confidence=0.0)
