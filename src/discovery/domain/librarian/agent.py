...
CAT_LIBRARIAN_PROMPT = (
    ...
    "<!-- isbn: {13자리 또는 10자리 ISBN 숫자, 확인 불가 시 이 줄 자체를 생략} -->\n\n"
    "규칙:\n"
    ...
    "8. ISBN 표기 (내부용, 사용자에게 절대 언급하지 마세요): 검색 결과에서 ISBN을 확인할 "
    "수 있다면 `- **추천 이유**:` 줄 바로 다음에 `<!-- isbn: 9788932917245 -->` 형식의 "
    "HTML 주석 줄을 추가하세요. 이 줄은 시스템이 내부적으로 페이지수를 정확하게 재검증하는 "
    "용도로만 쓰이며 사용자 화면에는 렌더링되지 않습니다. ISBN을 확인할 수 없으면 이 줄을 "
    "통째로 생략하세요(빈 값이나 추측한 숫자를 적지 마세요)."
)
