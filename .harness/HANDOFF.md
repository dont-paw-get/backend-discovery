...
## 2026-09-06 — CLIAR-305 국내 출간·2000년 이후 도서 추천 제한 및 사서 전환 버그 수정, PR #77 생성
- 브랜치: `CLIAR-305-domestic-recent-books-only` (`origin/develop`에서 분기).
- 배경: 추천 에이전트(웹 검색 기반)가 해외 원서·저작권 만료 고전문학을 추천해 알라딘 서지
  검증(`BookMetadataClient`)에서 ISBN을 못 찾는 사례가 잦다는 사용자 제보로 착수.
  프롬프트 규칙 추가로 개선 가능한지 검토 후 진행(레이턴시 단축 효과는 미미할 것이라고
  사전에 안내했고, 실측으로도 확인됨 — 목적은 품질/신뢰성 개선).
- `domain/librarian/agent.py`의 `CAT_LIBRARIAN_PROMPT`/`STORK_LIBRARIAN_PROMPT`에 규칙 10
  추가: 국내 서점(알라딘 등)에서 검색 가능한 정식 국내 출간(정발) 도서만 추천, 2000년
  이전 고전문학/고전소설 배제. `search_books` 검색어 구성 지침에도 "국내 출간"/"한국 정발"
  키워드 포함 유도 추가. `infrastructure/search/book_search_tool.py`의 `search_books` 도구
  docstring도 동일하게 보강.
- 로컬에서 실제 Bedrock(Haiku 4.5, MFA 임시 세션) + Tavily 호출로 검증:
  - "세계 명작 고전 문학 추천해줘"(블루) — 사용자가 명시적으로 고전을 요청하면 규칙을
    오버라이드하고 고전을 그대로 추천하는 예외 확인(오만과 편견, 변신, 햄릿). 일반 추천
    질의에서는 규칙이 잘 지켜짐(SF 추천 시 테드 창/박해울/김영탁 등 국내 정발 2010년대
    이후 작품만 나옴).
  - "해외 경영/리더십 신간 원서 추천해줘"(슈빌) — 국내 미정발 사유를 스스로 설명하고
    국내 정발/국내 저자 대안을 되묻는 안전한 동작 확인(가상 도서 지어내지 않음).
  - `RecommendBooksTool.recommend()` 전체 파이프라인(알라딘 검증 포함)으로도 국내 최신작
    필터링이 잘 동작함을 확인(다만 로컬은 backend-book 인증 토�29 없어 401 → graceful
    degradation만 확인, 알라딘 쪽수 덮어쓰기 자체는 dev 실토큰으로 재확인 필요).
- **부수 버그 발견 및 수정**: 시연 시나리오 재현 중 "고양이 사서로 바꿔줘"처럼 명시적
  사서 전환을 요청했을 때, 오케스트레이터가 `consult_librarian`을 호출하지 않고 스스로
  "제 권한 밖이라 못 바꿔드려요" 식의 거절 답변을 지어내는 버그를 발견함(프론트에 이미
  "블루로 전환"/"슈빌로 전환" 버튼이 있어 텍스트 요청도 동일하게 동작해야 함). 원인:
  `SHARED_GUARDRAILS`의 도구 라우팅 분기에 "사서 전환 명시적 요청"을 다루는 전용 분기가
  없어 LLM이 아무 도구도 안 부르고 임의 응답을 생성함(`evaluate_local_persona_response`
  로컬 엔진 자체의 `switch_to` 판단 로직은 원래부터 정확했음 — fallback 경로에서는
  문제없이 동작 확인). `domain/orchestrator/agent.py`의 `SHARED_GUARDRAILS`에 "사서
  전환(스위칭) 명시적 요청" 전용 분기를 추가해 `consult_librarian` 호출을 강제. 재실측으로
  `switch_to` 필드가 정상적으로 채워짐을 확인(다만 생성된 안내 문구가 "다음에 뵐 기회가
  있을 것" 같이 다소 회피성 — 기능은 정상이나 톤 다듬기는 이번 범위에서 보류, 사용자가
  속도를 우선해 스킵 지시).
- 시도했으나 반증되어 되돌린 것: 동기 `chat()` 경로에 스트리밍 경로의 `get_initial_meta`
  프리페치(사서 상담 사전 호출)를 이식해 "날씨 기반 추천" 2단 도구 연쇄의 LLM 사이클을
  줄이려 시도했으나, 실측 결과 오히려 소폭 악화(15.11초→19.64초)되어 전체 되돌림
  (`git checkout`). 원인: 프리페치는 도구 실행의 네트워크 왕복만 절약하고, LLM이 도구
  호출→결과 처리→재추론하는 사이클 수 자체는 줄이지 못함(로그상 `Tool #1`/`Tool #2`
  순서가 그대로 남아있었음).
  - 별도로 "시그널을 프롬프트에 텍스트로 사전 주입하고 `consult_librarian` 도구를
    아예 빼는" 실험(1회성 스크립트)은 15.11초→13.93초로 약 8% 개선 확인. 다만 이
    방식은 순수 대화/인사 분기(사서의 대화 메시지 자체가 필요한 케이스)와 구조적으로
    충돌해 오케스트레이터 라우팅 전체 재설계가 필요한 수준이라 이번 범위에서는
    적용하지 않고 코드에 반영하지 않았다(실험 스크립트만, 프로덕션 코드 변경 없음).
    향후 별도 티켓으로 검토할 후보로 남긴다.
- 프롬프트 축소(성능 개선 목적)는 실측으로 효과 없음이 확인되어 시도하지 않음 — 도구
  호출이 1회뿐인 케이스도 22초 이상 걸린 사례가 있어(STEP 3 실측), 병목이 프롬프트
  길이가 아니라 LLM 추론 자체/사이클 수임을 재확인.
- 검증: `ruff check .`/`mypy .` 90파일 통과, `pytest -m "not integration"` 318건 전체
  통과(신규 1건 포함, 무회귀).
- 커밋(`1abb55a`) 후 `origin/develop` 대상 PR #77 생성 완료
  (https://github.com/dont-paw-get/backend-discovery/pull/77).

### 다음 세션이 할 일
1. PR #77 리뷰/머지 확인.
2. dev 배포 후 실측: (a) 국내 출간·2000년 이후 필터링이 실사용 질의에서도 잘 동작하는지,
   (b) "고양이 사서로 바꿔줘"/"슈빌로 바꿔줘" 등 사서 전환 요청이 정상적으로 전환되는지
   프론트 버튼 클릭 없이 텍스트 요청만으로도 확인.
3. 사서 전환 안내 문구("다음에 뵐 기회가 있을 것" 등 회피성 표현) 톤 다듬기 여부 사용자와
   재논의 — 이번 세션은 기능(switch_to 필드) 확인만 하고 문구는 그대로 두었다.
4. 시연 스크립트(회원가입→로그인→UI→사서전환→채팅 3종 등) 전체 리허설은 이번 세션에서
   미완료 — MFA 세션이 매 `execute_bash` 호출마다 만료되어(독립 프로세스, 최대 지속시간
   짧음) 반복 실측에 어려움이 있었다. 사용자가 실제 로그인 세션(장시간 유지되는 자격증명)
   에서 직접 리허설하거나, 더 긴 MFA 세션 유지 방법을 찾는 게 필요.
5. "시그널 사전 주입 + consult_librarian 도구 제거" 방식(8% 개선 확인됨)을 실제로 적용할지
   여부는 오케스트레이터 라우팅 재설계 규모의 작업이라 별도 티켓 논의 필요.



## 2026-09-06 — CLIAR-305 후속: 사서 전환 결정론화 + ISBN 미검증 도서 제거 + 최신 유명작 유도
- 브랜치: `CLIAR-305-domestic-recent-books-only` (PR #77 머지 후 후속 커밋, 사용자가 305에
  이어서 하라고 지시).
- 배경: PR #77 배포본을 dev 라이브(CloudFront `d1wab52ln5by5k.cloudfront.net/api/v1/chat`)로
  실측하니 두 문제가 재현됨. (1) 사서 전환: `librarian_id=stork`에서 "블루로 바꿔줘" 6회 중
  정상 전환 2 / 거절 지어냄("전환 기능이 없다") 1 / 빈응답 3 — PR #77의 프롬프트 분기가
  Haiku 4.5의 확률적 도구 호출 누락에 의존해 비결정적. (2) 유저 제보(스크린샷): 데일 카네기
  자기관리론·부자의 생각법 등 해외 번역서가 노출됐는데, ISBN 검증 실패로 쪽수/장르가 빈
  카드가 섞임. 유저 확정 요구: "바꿔줘"로 전환 시연할 거니 전환은 결정론적으로 확실히 /
  외국 도서 자체는 OK지만 ISBN 검증 안 되는 책은 빼고 / 장르 칩 제대로 / 해외면 유명·최신작
  위주(시연 그림).
- **캐시 관련 사전 논의(코드 변경 없음)**: 유저가 "캐시 워밍/프롬프트 캐시 TTL 연장" 가능성을
  물어봄. 확인 결과 — (a) dev는 이미 `ENABLE_PROMPT_CACHING=true`(k8s/base/configmap.yaml,
  코드 기본값은 False지만 configmap이 덮어씀). (b) Bedrock 프롬프트 캐시 TTL은 AWS API상
  `5m`/`1h` 지원하지만, 우리가 쓰는 **strands-agents 1.26.0의 BedrockModel이 cachePoint에
  `type`만 넣고 `ttl` 필드를 하드코딩으로 제외**(`.venv/.../strands/models/bedrock.py:496`)해
  항상 5분 고정 — config로 못 늘림. (c) 프롬프트 캐시가 줄이는 건 입력 처리(수백ms~1초)뿐,
  체감 15~24초는 출력 생성이라 캐시로 해결 안 됨. (d) Redis 캐시(서지/장르 30일, Tavily
  검색)가 히트 안 되는 건 프롬프트 캐시 TTL과 무관하고, "LLM 출력이 비결정적이라 검색어/
  추천 책이 매번 달라져 캐시 키가 재현 안 됨"이 원인(추정, 로그 실측 필요). 시연 예열은
  Redis 캐시(30일)는 전날 밤 해두면 유지되지만 프롬프트 캐시(5분)는 시연 직전에만 유효.
- 구현(Task 1~4, 세부는 STATE.md):
  - Task 1: `domain/orchestrator/switch_gate.py` 신설. `detect_switch_request`가 현재 활성
    사서 기준으로 "상대 사서" 호칭+전환 서술어를 감지(자기 자신 호칭은 전환 아님).
    `evaluate_switch_gate`가 감지 시 `evaluate_local_persona_response`(이미 정확히 동작)를
    직접 태워 `LibrarianResponse`(switch_to 포함) 반환. `OrchestratorService.chat`/
    `stream_chat`(guardrail 게이트 뒤)·`get_initial_meta`(consult 앞)에 배선. chat은 즉시
    반환, stream_chat은 전환 문구를 단일 청크로 yield, get_initial_meta는 X-Switch-To 헤더용.
    셋 다 세션 메타 `librarian_id`를 대상 사서로 갱신.
  - Task 2: `_resolve_isbn_pages_and_genre` 반환을 `(isbn, pages, genre_response)`로 확장.
    `_verify_page_counts`가 isbn=None인 도서를 `_remove_book_block`로 제거. 단
    `auth_token`이 있고 `len(unverified) < len(titles)`(검증된 책 하나 이상 남음)일 때만
    제거 — 무인증(알라딘 401 전량 실패)·전량 실패 시 0권 방지 위해 스킵. `_remove_book_block`
    헬퍼 신설.
  - Task 3: CAT/STORK 프롬프트 규칙 10을 "국내 정발만/해외 원서 금지"에서 "국내 검색 가능·
    유명·최신작 우선(정발 번역서 허용, 무명/미출간 회피, 명시적 고전 요청 시 예외)"로 완화
    (replace_all). `test_librarian_agent.py`의 해당 assert 갱신. search_books 검색어 유도
    문구(국내 출간/한국 정발)는 ISBN 검증 가능성을 높이므로 유지.
  - Task 4: `test_switch_gate.py` 13건 신설(양방향 전환/자기호칭 오탐/도메인만은 미발동/
    switch_to 반환), `test_recommend_tool.py`에 ISBN 필터 3건 추가. ruff/mypy(92파일)/
    `pytest -m "not integration"` 331건 전체 통과.
- 커밋·push는 아직 안 함(사용자 승인 대기). 커밋 시 `[CLIAR-305]` 태그.

### 다음 세션이 할 일
1. 커밋/push 승인 시 진행(`[CLIAR-305]`). 305 브랜치는 이미 원격에 있고 PR #77이 머지됐으므로,
   후속 커밋을 push하면 새 PR을 열거나 기존 흐름에 맞춰 처리(사용자 확인).
2. **dev 배포 후 실토큰(데모 로그인 세션)으로 라이브 재현 필수**:
   - 사서 전환: "블루/슈빌로 바꿔줘" 10회 반복해 100% `switch_to` 채워지는지(비결정성 사라졌는지).
   - ISBN 필터: 외국 도서 추천 시 쪽수·장르 없는 카드가 사라지는지, 검증된 책만 남는지.
     **무인증(더미 토큰)으로는 알라딘 401이라 필터가 스킵되므로 검증 불가 — 반드시 실토큰 필요.**
   - 장르 칩: 추천 카드에 장르가 NONE 없이 채워지는지.
