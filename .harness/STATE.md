...
| CLIAR-40 핵심 코드 구현 | 🔶 진행 중 | Task 5(`books` 모델), Task 6(DTO+BookRepository), Task 7(Bedrock Mocking — Protocol, Mock/실 클라이언트 둘 다 구현, `LLM_PROVIDER` 스위치, 단위 테스트 15건 — 실 AWS 호출 없이 `boto3.client` patch로 검증) 완료. Task 8 남음 |
