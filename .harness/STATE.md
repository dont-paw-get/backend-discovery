...
| CLIAR-51 추천 에이전트 (Strands 기반) | 🚧 진행 중 | Task 1(strands-agents 팩토리)·Task 2(BookSearchTool Tavily 검색 도구)·Task 3(LibrarianService 세션 연동 및 도구 배선, 단위 테스트 4건)·Task 4(POST /chat 및 /api/v1/chat 라우터, 스트리밍 응답, CORS 설정, openapi.yaml 계약, 라우터 단위 테스트 4건) 모두 완료. 전체 단위 테스트 22건 통과. Task 5(프롬프트 캐싱 검토) 및 Task 6(E2E 검증) 진행 예정 |
